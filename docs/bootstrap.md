...
このリポジトリでは [`templates/cluster-inputs.env.example`](/home/azuki/work/mistship/templates/cluster-inputs.env.example) を平文テンプレートとして置き、実運用の cluster input は `secrets/mistship/cluster-inputs.sops.env` と `secrets/mistship/cluster-secrets.sops.yaml` に暗号化して置きます。

まず SOPS で cluster input を復号します。
...
平文テンプレートとして必要なキーは [`templates/cluster-inputs.env.example`](/home/azuki/work/mistship/templates/cluster-inputs.env.example) を参照します。復号後は `.secret/cluster-inputs.env` に最低限、以下が入っている前提です。
...
`.secret` 配下は Git に含めず、`.secret/.gitkeep` だけを置きます。SOPS 管理の詳細は [`docs/secrets.md`](/home/azuki/work/mistship/docs/secrets.md) を参照してください。
...
- Git に載せる: [`image.yml`](/home/azuki/work/mistship/image.yml)、[`patches/common.yaml`](/home/azuki/work/mistship/patches/common.yaml)、[`patches/controlplane.yaml`](/home/azuki/work/mistship/patches/controlplane.yaml)、[`patches/worker.yaml`](/home/azuki/work/mistship/patches/worker.yaml)、SOPS で暗号化した `cluster-inputs.sops.env` と `cluster-secrets.sops.yaml`
- Git に載せない: `cluster-secrets.yaml`、`talosconfig`、`kubeconfig`、生成済みの `controlplane.yaml`、`worker.yaml`

`patches/*.yaml` には公開可能な設定だけを書き、復号済み secret は `.secret/cluster-secrets.yaml` に閉じ込めます。
...
公開可能な差分は `patches/*.yaml` に寄せ、secret は SOPS 管理した `cluster-secrets.sops.yaml` を `.secret/cluster-secrets.yaml` へ復号して使います。
...
worker を追加する場合は、同じ `.secret/cluster-secrets.yaml` から `worker.yaml` を使って TalOS に machine config を適用します。
